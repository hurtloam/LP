 ...
 *
 * @details When two lines of text needs to be handled as one object we contain them in a MenuItem object.
 *
 * @param first_obj
 * @param second_obj
 * @param id
 ...
 * @param TextureID
 */
MenuItem::MenuItem(TextObject *first_obj, TextObject *second_obj, int id, glm::vec3 scale, ObjectType objType, GLuint texture, glm::vec3 trans, LPWatch* watch, ResObj& resObj,
        ...
        second_obj)
{
		...
		textObjectFirst->setTrans(trans - textObjectFirst->getTrans() + glm::vec3(0.0f, 0.0f, 10.1f));
		textObjectSecond->setTrans(trans - textObjectSecond->getTrans() - glm::vec3(0.0f, 0.3f, 0.0f) + glm::vec3(0.0f, 0.0f, 10.1f));
...
void MenuItem::setTrans(glm::vec3 tr) {
	StaticObj::setTrans(tr);
	textObjectFirst->setTrans(tr);
	textObjectSecond->setTrans(tr);
}

void MenuItem::setColor(GLfloat const _color[4])
{
//	float c[4];
//	c[0] =_color[0];
//	c[1] =_color[1];
//	c[2] =_color[2];
//	c[3] =_color[3];
	std::cout << "SetColor for MenuItem " << endl;
