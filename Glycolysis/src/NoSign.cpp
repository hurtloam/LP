/*
 * NoSign.cpp
 *
 *  Created on: Dec 1, 2015
 *      Author: peter
 */

#include "../inc/NoSign.h"
#include <BulletCollision/CollisionShapes/btBoxShape.h>
//#include <BulletCollision/CollisionShapes/btSphereShape.h>
#include <BulletDynamics/Dynamics/btRigidBody.h>
#include <glm/glm.hpp>
#include <glm/detail/type_vec3.hpp>
#include <GL/glew.h>
#include <LinearMath/btDefaultMotionState.h>
#include <LinearMath/btQuaternion.h>
#include <LinearMath/btScalar.h>
#include <LinearMath/btTransform.h>
#include <LinearMath/btVector3.h>
#include "../inc/ResObj.h"

NoSign::NoSign(TextObject *first_obj, TextObject *second_obj, int id, glm::vec3 scale, ObjectType objType, GLuint texture, glm::vec3 trans, LPWatch* watch, ResObj& resObj,
                const View& view, LPUniform lpUniform) : StaticObj(
id, scale, objType, texture, trans, watch, resObj, view, lpUniform), textObjectFirst (first_obj), textObjectSecond (
        second_obj)
{

		textObjectFirst->setTrans(trans - textObjectFirst->getTrans());
		textObjectSecond->setTrans(trans - textObjectSecond->getTrans() + glm::vec3(0.2f, 0.0f, 0.0f));


		b_motionState =
				new btDefaultMotionState(btTransform(btQuaternion(0, 0, 0, 1), btVector3(trans.x, trans.y, trans.z)));

		b_shape = new btBoxShape(btVector3(1.0,0.4,0.5));
		btScalar mass = 0;
		btRigidBody::btRigidBodyConstructionInfo b_rigidBodyCI(mass, b_motionState, b_shape);



		b_rigidBody = new btRigidBody(b_rigidBodyCI);

	//	b_rigidBody->setSleepingThresholds (0.0f, 0.0f);
	//	b_rigidBody->setAngularFactor (0.0f);

	//	b_rigidBody->setCollisionFlags(b_rigidBody->getCollisionFlags() | btCollisionObject::CF_KINEMATIC_OBJECT);
		b_rigidBody->setActivationState(DISABLE_DEACTIVATION);
		GLfloat blackish[4] = {0,0,0,1};
		setColor(resObj.black);


counter = 0;
}

void NoSign::setTrans(glm::vec3 tr) {
	StaticObj::setTrans(tr);
	textObjectFirst->setTrans(tr);
	textObjectSecond->setTrans(tr);
}

void NoSign::setColor(GLfloat const _color[4])
{
	std::cout << "SetColor for signPost " << endl;
	textObjectFirst->setColor(_color);
	textObjectSecond->setColor(_color);
}

NoSign::~NoSign() {
	// TODO Auto-generated destructor stub
}

void NoSign::setNo(int no)
{
	int first;
	int second;

	if (no < 10)
	{
		first = 0;
		second = no;
	}
	else if (no < 20)
	{
		first = 1;
		second = no - 10;
	} else if (no < 30)
	{
		first = 2;
		second = no - 20;
	} else if (no < 40)
	{
		first = 3;
		second = no - 30;
	} else if (no < 50)
	{
		first = 4;
		second = no - 40;
	}
	obj_typeFirst =	setInternalNo(first);
	obj_typeSecond = setInternalNo(second);
}
ObjectType NoSign::setInternalNo(int no)
{
//	_objtype = 1036 + no;
	switch(1036 + no)
	{
	case 1036:
		return ObjectType::txt_36;
	case 1037:
		return ObjectType::txt_37;
	case 1038:
		return  ObjectType::txt_38;
	case 1039:
		return ObjectType::txt_39;
	case 1040:
		return ObjectType::txt_40;
	case 1041:
		return ObjectType::txt_41;
	case 1042:
		return ObjectType::txt_42;
	case 1043:
		return ObjectType::txt_43;
	case 1044:
		return ObjectType::txt_44;
	case 1045:
		return ObjectType::txt_SELECT_OBJECT;
	default:
		break;
	}
	cout << "SetNo - value=" << no <<  endl;
}

void NoSign::draw() {
	StaticObj::draw(cube);
	textObjectFirst->draw(obj_typeFirst);
	textObjectSecond->draw(obj_typeSecond);
}

void NoSign::move(bool flag) {

  textObjectFirst->move (flag);
  textObjectSecond->move (flag);

	//	 for the physics
		btTransform trans;
		btVector3 bvec;
		getRigidBody()->getMotionState()->getWorldTransform(trans);
		bvec = trans.getOrigin();


		// this made him move push stuff around
		trans.setOrigin(btVector3(getTrans().x, getTrans().y, getTrans().z));

		getRigidBody()->getMotionState()->setWorldTransform(trans);
		getRigidBody()->setCenterOfMassTransform (trans);
}
