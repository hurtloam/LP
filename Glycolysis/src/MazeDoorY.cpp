...
MazeDoorY::MazeDoorY (int id,
                      ObjectType obj,
                      glm::vec3 shape,
                         glm::vec3 scale, GLuint texture,
                          glm::vec3 trans, LPWatch* watch, ResObj& resObj,
                          const View& view, 
		LPUniform lpUniform) : StaticObj(id, scale, obj, texture, trans, watch, resObj, view, lpUniform)
{
...
void
MazeDoorY::setOpenDoor () {
  std::cout << "ENTER MazeFloor::setOpenDoor" << std::endl;
  openDoor = true;
}
void
MazeDoorY::move (std::vector<std::pair<int, int>> collisionDetected, glm::vec3 normal) {
//  std::cout << "ENTER MazeFloor::move" << std::endl;
  if (openDoor)
  {
    ...
    if (++delay < 190)
    {
      ...
      setTrans (glm::vec3 (0.0f, 0.f, 0.02f));
    }
    else
    {
